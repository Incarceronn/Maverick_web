...
def answer(question, context, co, model, chat_history=""):
  ...
  if 'command' in model:
    ...
    if chat_history:
      prompt = (
        "This is an example of factual question answering chat bot. It "
  ...
  # print('TOKENISED PROMPT in ans.py: ' + prompt)
  ...
  generations = [[
    trim_stop_sequences(prediction.generations[i].text.strip(),
                        stop_sequences), prediction.generations[i].likelihood
...
def answer_with_search(question,
  ...
  paragraphs, paragraph_sources = get_results_paragraphs_multi_process(
    question, serp_api_token, url=url)
  if not paragraphs:
    return ("", "", "")
  sample_answer = get_sample_answer(question, co)

  results = embedding_search(paragraphs,
                             paragraph_sources,
                             sample_answer,
                             co,
