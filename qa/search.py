...
def get_paragraphs_text_from_url(k):
  ...
  try:
    ...
    pretty_print(
      "FAIL",
      f"ERROR: Page '{search_result_url}' could not be loaded! Exception message: {e}"
    )
    return []


def get_results_paragraphs_multi_process(search_term,
                                         serp_api_token,
                                         url=None):
  ...
  pool = Pool(len(urls))
  multiple_results = [
    pool.apply_async(get_paragraphs_text_from_url, args=(url, ))
...
def embedding_search(paragraphs,
                     paragraph_sources,
                     search_term,
                     co,
  ...
  embeddings = co.embed(texts=paragraphs + [search_term],
                        model=model,
  ...
  distances = [
    cosine_similarity(x, search_embedding) for x in paragraph_embeddings
  ]
  # print(
  #   'result of embed search: ',
  #   sorted(list(zip(paragraphs, paragraph_sources, distances)),
  #          key=lambda x: x[2]))
  return sorted(list(zip(paragraphs, paragraph_sources, distances)),
                key=lambda x: x[2])
